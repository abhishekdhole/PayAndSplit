...
public class cashPayment extends AppCompatActivity {
    ...
    void fetcheuserinformation()
    {
        ...
        void addCash(String amount, String upiId, String name, String note) {
                ...
                myRef.child(GroupID).child(userID).child("Payment Logs").child(uniqueID).child("Ammount").setValue(finalValue);
                myRef.child(GroupID).child(userID).child("Payment Logs").child(uniqueID).child("Note").setValue(Note);
                //myRef.child(userID).child("Payment Logs").child(uniqueID).child("Payment_ID").setValue(uniqueID);
                myRef.child(GroupID).child(userID).child("Payment Logs").child(uniqueID).child("UPI_ID").setValue(upiid);


                myRef.child(userID).child("Payment Logs").child(uniqueID).child("Ammount").setValue(finalValue);
                myRef.child(userID).child("Payment Logs").child(uniqueID).child("Note").setValue(Note);
                //myRef.child(userID).child("Payment Logs").child(uniqueID).child("Payment_ID").setValue(uniqueID);
                myRef.child(userID).child("Payment Logs").child(uniqueID).child("UPI_ID").setValue(upiid);


                toastMessage("Adding " + GroupID + " to database...");

                Intent intent = new Intent(cashPayment.this,homepage.class);
